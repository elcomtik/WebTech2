...
if (isset($_POST['linkInsert'])) {
    $sql = "INSERT INTO `PUBLICATIONS_LINKS`(`NAME`, `LINK`) VALUES ('" . $_POST['name'] . "', '" . $_POST['link'] . "')";
    $conn->query($sql);
    $_POST['linkInsert'] = array();
} else if (isset($_POST['categoryInsert'])) {
    $sql = "INSERT INTO `PUBLICATIONS_DOCUMENTS`(`DOCUMENT`) VALUES ('" . $_POST['name'] . "')";
    $conn->query($sql);
    $_POST['categoryInsert'] = array();
} else if (isset($_POST['attachmentInsert'])) {
    $targetfolder = "uploads/publications/";
    $targetfolder = $targetfolder . basename($_FILES['file']['name']);
    move_uploaded_file($_FILES['file']['tmp_name'], $targetfolder);
    $sql = "INSERT INTO `PUBLICATIONS_ATTACHMENTS`(`ATTACHMENT`, `ATTACHMENT_LINK`, `DOCUMENT_ID`) VALUES ('" . $_POST['name'] . "', '" . $targetfolder . "', " . $_POST['category'] . ")";
    $conn->query($sql);
...
<div id="main">
    <h1><?php echo $lang['intranet.publications'] ?></h1>
    <table class="table">
        <thead>
        <tr>
        ...
        <?php
        $sql = "SELECT ID, NAME, LINK FROM PUBLICATIONS_LINKS";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr><th colspan='2'><a href='" . $row['LINK'] . "' target='_blank' contenteditable='true' onBlur='saveToDatabase(this, \"NAME\"," . $row['ID'] . ", \"PUBLICATIONS_LINKS\")'>" . $row['NAME'] . "</a></th></tr>";
            }
        }
        ?>
        </thead>
        ...
        $sql = "SELECT ID, DOCUMENT FROM PUBLICATIONS_DOCUMENTS";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            $documents = array();
            while ($row = $result->fetch_assoc()) {
            ...
            foreach ($documents as $document) {
                $sql = "SELECT ID, ATTACHMENT, ATTACHMENT_LINK FROM PUBLICATIONS_ATTACHMENTS WHERE DOCUMENT_ID='" . $document->id . "'";
                $result = $conn->query($sql);
                echo "<tr>";
                echo "<td  contenteditable='true' onBlur='saveToDatabase(this, \"DOCUMENT\"," . $document->id . ", \"PUBLICATIONS_DOCUMENTS\")'>" . $document->document . "</td>";
                echo "<td>";
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<a href='" . $row['ATTACHMENT_LINK'] . "' target='_blank' contenteditable='true' onBlur='saveToDatabase(this, \"ATTACHMENT\"," . $row['ID'] . ", \"PUBLICATIONS_ATTACHMENTS\")'>";
